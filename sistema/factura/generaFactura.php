<?php
	...
	if(empty($_REQUEST['cl']) || empty($_REQUEST['f']))
	{
		...
		$pdf->Ln();
		$pdf->image("img/membrete1.png", 50, 18, 15, 15, 'PNG');
		$pdf->SetFont('Arial', 'B', 7);
		$pdf->Cell(15, 5, "CI/NIT: ", 0, 0, 'L');
		$pdf->SetFont('Arial', '', 7);
		...
		$pdf->Cell(15, 5, 'Son:', 0, 1, 'L');

		//Incluímos la clase pago
		$totalpagar=utf8_decode(number_format($result_venta['totalfactura'], 2, '.', ','));
		require_once "CifrasEnLetras.php";
		$v=new CifrasEnLetras(); 
		//Convertimos el total en letras
		$letra=($v->convertirEurosEnLetras($totalpagar));

		$pdf->Cell(42, 5, utf8_decode($letra), 0, 0, 'L');
